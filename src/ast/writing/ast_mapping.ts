...
class StructuredDocumentationWriter extends ASTNodeWriter {
    static render(text: string, formatter: SourceFormatter, useJsDocFormat?: boolean): string {
        ...
        // very lazy fix
        return (prefix + documentation).replace(/\/\/\/\s\s+/g, "/// ");
    }
...
class ModifierInvocationWriter extends ASTNodeWriter {
    writeInner(node: ModifierInvocation, writer: ASTWriter): SrcDesc {
        return writer.desc(
            ...
            node.kind === ModifierInvocationKind.BaseConstructorSpecifier
                ? ["(", ...join(node.vArguments, ","), ")"]
                : [])
        );
    }
}
...
class YulLiteralWriter extends ASTNodeWriter {
    writeInner(node: YulLiteral): SrcDesc {
        ...
        if (node.kind === YulLiteralKind.String) {
            result = node.value ? JSON.stringify(node.value) : `hex"${node.hexValue}"`;
            // throw Error(JSON.stringify({ value: node.value, hexValue: node.hexValue }));
        } else {
            result = node.value;
        }
