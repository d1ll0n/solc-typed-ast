...
export function yulToStringExpression(a: YulExpression): string {
    ...
    if (a instanceof YulIdentifier) {
        return isConstant(a)
            ? evalConstantExpr(a, new InferType(LatestCompilerVersion)).toString()
            : a.name;
    }
    if (a instanceof YulTypedName) return a.name;
    if (a instanceof YulLiteral)
        return evalConstantExpr(a, new InferType(LatestCompilerVersion))?.toString();
